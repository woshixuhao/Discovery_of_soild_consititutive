...
def _ordered_vars(expr,with_x=True):
    # 固定顺序：x, A, B 在前，其余符号按 sort_key 排序
    if with_x:
        return [x, A, B] + sorted(
            [s for s in expr.free_symbols if s not in {x, A, B}],
...
class Random_graph_for_expr():
    ...
    def generate_random_graph(self,max_terms=3):
        ...
        if n_terms == 1:
            ...
            for i in range(n_terms):
                graph=self.concate_subgraph_to_node(graph, self.generate_graph_template(), 0, 'add')
        return graph
...
class Genetic_algorithm(Random_graph_for_expr,Graph_to_sympy):
    ...
    def get_regressed_function(self,graph):
        ...
        variables= _ordered_vars(expr,with_x=False)
        #variables = list(expr.free_symbols)
    ...
    def distinction(self,graphs):
        for i in range(1,len(graphs)):
            graphs[i]=self.generate_random_graph(max_terms=self.max_terms)
        return graphs
    ...
    def evolution(self,save_dir='default'):
        ...
        if self.use_parallel_computing==True:
            for i in range(self.size_pop):
                graph=self.generate_random_graph(max_terms=self.max_terms)
        ...
        for iter_num in tqdm(range(self.generation_num)):
            ...
            if self.use_parallel_computing==True:
                ...
                pickle.dump(new_graphs, open(f'result_save/new_graphs.pkl', 'wb'))
            ...
            for index in range(self.size_pop-len(sorted_fitness)):
                sorted_graph.append(self.generate_random_graph(max_terms=self.max_terms))
